...
import { PanelLeft, Lock, Unlock } from "lucide-react"
...
>(
  (
    ...
    const toggleLock = React.useCallback(() => {
        ...
        document.cookie = `${SIDEBAR_LOCK_COOKIE_NAME}=${newLockState}; path=/; max-age=${SIDEBAR_COOKIE_MAX_AGE}`;
    }, [isLocked]);
...
>(
  (
    ...
    const { isMobile, setOpen, openMobile, setOpenMobile, isLocked, state } = useSidebar()
    ...
    return (
      <div
        ref={ref}
        onMouseEnter={() => !isLocked && setOpen(true)}
...
const SidebarHeader = React.forwardRef<
  ...
  React.ComponentProps<"div">
>(({ className, ...props }, ref) => {
  const { state, toggleLock, isLocked } = useSidebar();
  return (
    <div
      ref={ref}
      data-sidebar="header"
      className={cn("flex h-14 items-center justify-between gap-2 p-2 relative",
    ...
    >
       ...
       <Button
        ...
        size="icon"
        className={cn(
          "absolute top-1/2 -translate-y-1/2 right-2 h-7 w-7",
          state === 'collapsed' && "hidden"
        )}
        onClick={toggleLock}
      >
        {isLocked ? <Lock className="h-4 w-4" /> : <Unlock className="h-4 w-4" />}
        <span className="sr-only">{isLocked ? 'Unlock' : 'Lock'} sidebar</span>
      </Button>
    </div>
  )
})
